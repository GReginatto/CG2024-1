...
float findBlocker(vec3 projectedTexcoord, float currentDepth, int samples) {
...
float computePenumbra(float blockerDepth, float currentDepth, float lightRadius) {
    return (currentDepth - blockerDepth) / blockerDepth * lightRadius;
}

float calculateShadowPCSS(vec3 projectedTexcoord, float currentDepth) {
    float avgBlockerDepth = findBlocker(projectedTexcoord, currentDepth, 8);
    if (avgBlockerDepth == -1.0) {
        return 1.0; 
    }

    float penumbraSize = computePenumbra(avgBlockerDepth, currentDepth, u_lightRadius);

    int filterSamples = int(clamp(penumbraSize * 32.0, 1.0, 64.0)); 
    ...
    for (int x = -filterSamples; x <= filterSamples; ++x) {
        for (int y = -filterSamples; y <= filterSamples; ++y) {
            ...
            float shadowDepth = texture(u_projectedTexture, projectedTexcoord.xy + offset).r;
            shadow += currentDepth - u_bias > shadowDepth ? 0.0 : 1.0;
        }
    }

    return shadow / float((filterSamples * 2 + 1) * (filterSamples * 2 + 1));
}

void main() {
    vec3 normal = normalize(v_normal);
    float lightIntensity = max(dot(normal, u_reverseLightDirection), 0.0);

    vec3 projectedTexcoord = v_projectedTexcoord.xyz / v_projectedTexcoord.w;
    float currentDepth = projectedTexcoord.z + u_bias;

    float shadowLight = calculateShadowPCSS(projectedTexcoord, currentDepth);

    vec4 texColor = texture(u_texture, v_texcoord) * u_colorMult;
    outColor = vec4(texColor.rgb * lightIntensity * shadowLight, texColor.a);
}
...
function main() {
  ...
  const depthTextureSize = 8192;  // Resolução aumentada
